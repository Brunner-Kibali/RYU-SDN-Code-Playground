...
class ProjectController13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # self.neighbor = defaultdict(dict)  # Neighbors, the structure is this: {SwithId: {Neighbour1:Port_Switcha,Neighbour2:Port_Switcha}}
        # self.bandwidth = defaultdict(lambda: defaultdict(lambda: DEFAULT_BW))  # Structure the same as before, calling for two values only this time [switch][port]
    ...
    def get_paths(self, src, dst):
        ...
        queue = [(src, [src])]
        paths = []  # If we use a generator make a comment.
        while queue:
            (edge, path) = queue.pop()
            for vertex in set(self.adjacency[edge].keys()) - set(path):
                if vertex == dst:
...
""" 
# Total the link costs of the links in the path.
    def get_link_and_path_cost(self, path):
        ...
        for i in range(len(path) - 1):
            ...
            # bl = min(self.bandwidth[path[i]][port1], self.bandwidth[path[i + 1]][port2])
            bl = self.bandwidth[path[i]][port1]
            ew = REFERENCE_BW / bl
            path_cost.append(ew)
            # path_cost.append(bl)
